...
- [x] **AC-1 (Tag Creation & Linking)**: Given an authenticated user creating or editing an issue, when tag names are provided (e.g., "bug", "frontend"), then the tags are created if not existing, associated with the issue, and stored in SQLite.
- [x] **AC-2 (Tag Detachment)**: Given an existing issue with associated tags, when edited to remove a tag, then the tag association is removed from the issue while preserving the tag entity in the database.
- [x] **AC-3 (Tag Display on List & Detail)**: Given issues with associated tags, when navigating to `/issues` or `/issues/[id]`, then the corresponding tag badges are rendered with distinct styling.
- [x] **AC-4 (Keyword Search by Title & Description)**: Given a search query `q` entered in the search bar, when submitted or updated, then the issue list filters to only issues whose `title` or `description` contains the query string (case-insensitive).
- [x] **AC-5 (Filter Issues by Tag)**: Given a selected tag filter (e.g., `?tag=bug`), when applied, then only issues associated with that tag are displayed in the list.
- [x] **AC-6 (Combined Multi-Criteria Filtering)**: Given a search query, a tag filter, and a status filter (e.g., `?q=auth&tag=security&status=OPEN`), when applied simultaneously, then the results satisfy all active criteria.
- [x] **AC-7 (Empty Search/Filter State)**: Given search or filter criteria that match zero issues, when viewed, then a clear empty state message is shown with an option to reset filters.
- [x] **AC-8 (Tag Validation & Normalization)**: Given tag inputs with leading/trailing whitespace or mixed casing, when submitted, then tag names are trimmed and normalized to prevent duplicate near-identical tags.
